...
namespace CSV2Mesh
{
    public class CSV2Mesh : EditorWindow
    {
            ...
            if (dataTypes)
            {
                ...
                if (GUI.Button(botton, GUIContent.none))
                {
                    foreach (var item in textAssets)
                    {
                        ...
                        ImportMeshFromRenderDoc(path, rotationN90, reverseUvY, reverseTris,Head, item.name);
        ...
        public static int ImportMeshFromRenderDoc(string assetPath, bool needRotationN90, bool reverseUvY, bool reverseTris, List<string> head, string name)
        {
            if (!System.IO.File.Exists(assetPath))
            {
                return -1;
            ...
            CreateMeshAssetAndShow(mesh,assetPath, name);
            return 0;
        }

        private static void CreateMeshAssetAndShow(Mesh mesh,string assetPath, string name)
        {
