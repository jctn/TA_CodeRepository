...
public class ParticleCloud : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        {
            mMatCapSphereMat = new Material(Shader.Find("Unlit/MatCapSphere"));
        }
        IniteMatrix();
    ...
    {
#if UNITY_EDITOR
        if (ShowTipSphere && MatCapSphereMesh != null && LightTex != null && EdgeLightTex)
        {
            UpdateTipSphere(ref mLightMatCapTipSphere, LightTex, "LightMatCapTipSphere", LightMatCapAngle, Vector3.zero);
            UpdateTipSphere(ref mEdgeLightTipSphere, EdgeLightTex, "EdgeLightMatCapTipSphere", EdgeLightAngle, new Vector3(0f, 2f, 0f));
        }
#endif
    }
    ...
    void IniteMatrix()
    {
        mLightMatCapP = Matrix4x4.Ortho(-0.495f, 0.495f, -0.495f, 0.495f, -1f, 0f);
        mEdgeLightP = Matrix4x4.Perspective(60f, 1f, 0.01f, 1f);
        mFogP = Matrix4x4.Perspective(60f, 2f, 0.01f, 1f);
    }
    ...
    void UpdateMatrix(ref Matrix4x4 mvp, Vector3 modelR, bool updateView, in Matrix4x4 p, Quaternion camR)
    {
        ...
        Matrix4x4 model = Matrix4x4.Rotate(transform.rotation) * Matrix4x4.Rotate(Quaternion.Euler(modelR));
        mvp = p * mview * model;
    }
    ...
    void RenderMatCaps(ScriptableRenderContext context, Camera[] cams)
    {
        ...
        {
            UpdateMatrix(ref mLightMatCapMVP, LightMatCapAngle, true, in mLightMatCapP, cams[0].transform.rotation);
            UpdateMatrix(ref mFogMVP, LightMatCapAngle, false, in mFogP, cams[0].transform.rotation);
            UpdateMatrix(ref mEdgeLightMVP, EdgeLightAngle, false, in mEdgeLightP, cams[0].transform.rotation);
        ...
        else
        {
            ...
            Camera cam = Camera.main != null ? Camera.main : editorCam;
            if(cam != null)
            {
